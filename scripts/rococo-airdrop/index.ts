...
async function getEligigbleAddresses(): Promise<{ address: string, balance: string }[]> {
...
async function airdropToAccounts() {
    ...
    await writeFile('failed_airdrops.json', JSON.stringify(failedAirdrops, null, 2), 'utf8');
}
